...
- `WEBHOOK_IP_ALLOWLIST` — список разрешенных IP/CIDR ЮKassa через запятую.
...
- Возможные причины, почему оплата прошла, а подписка не продлилась:
  - Вебхук не доходит (неверный `WEBHOOK_URL`, нет HTTPS/порт 443/8443).
  - Nginx/прокси возвращает 502 или не проксирует путь `/webhook`.
  - Вебхук заблокирован по IP (не настроен `WEBHOOK_IP_ALLOWLIST`).
  - Не совпадает `WEBHOOK_SECRET` (проверяется по заголовку `Authorization`).
  - Платеж создан без metadata `user_id` (подписка не привязана).
- Если подписка закончилась, в личном кабинете блокируется создание и просмотр ссылок до оплаты.
